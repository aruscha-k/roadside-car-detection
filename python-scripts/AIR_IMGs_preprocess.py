...
from helpers_geometry import calculate_slope, get_y_intercept, find_angle_to_x, calc_perpendicular, \
    calc_interception_of_two_lines, calculate_start_end_pt
...
if __name__ == "__main__":
    ...
    tif_file = PATHS.DATASET_FOLDER_PATH + "TDOP_2022_RGB_10cm_Testgebiet.tif"

    # segment data from Leipzig as pandas DF
    with open(PATHS.DATASET_FOLDER_PATH + "testgebiet1_segments_df.pkl", 'rb') as f:
        segments_df = pickle.load(f)

    # folders to save IMGs in
    cropped_folder = PATHS.IMG_FOLDER_PATH + 'cropped_imgs/'
    rotated_folder = PATHS.IMG_FOLDER_PATH + 'cropped_rotated/'
    ...
    for idx, row in segments_df.iterrows():
        ...
        street_width = 20
        # checking if there is a row named cat

        if 'cat' in segments_df.columns:
            if row['cat'] == "W":
                continue
